...
Class WebRemocon
                            ...
                            If s.Length = 9 Then
                                If IsNumeric(s(1)) And IsNumeric(s(5)) And IsNumeric(s(7)) Then 'サービスID,TSIDが数値なら
                                    If Val(s(8)) > 0 Then
                                        ...
                                        End If
                                        html &= "<option value=""" & bondriver & "," & s(5) & "," & chspace_add_tsid.ToString & """>" & s(0) & "</option>" & vbCrLf
                                        'Else
    ...
    Public Sub start_movie(ByVal num As Integer, ByVal bondriver As String, ByVal sid As Integer, ByVal ChSpace As Integer, ByVal udpApp As String, ByVal hlsApp As String, hlsOpt1 As String, ByVal hlsOpt2 As String, ByVal wwwroot As String, ByVal fileroot As String, ByVal hlsroot As String, ByVal ShowConsole As Boolean, ByVal udpOpt3 As String, ByVal filename As String, ByVal NHK_dual_mono_mode_select As Integer, ByVal Stream_mode As Integer, ByVal resolution As String, ByVal VideoSeekSeconds As Integer, ByVal nohsub As Integer, ByVal baisoku As String, ByVal hlsOptAdd As String, ByVal margin1 As Integer, ByVal hlsAppSelect As String, ByVal profileSelect As String, ByVal httpApp As Integer, ByVal iso As Object)
        ...
        If ChSpace >= 100 Then
            tsid = Int(ChSpace / 100)
            ChSpace = ChSpace Mod 100
            If tsid > 0 Then
                log1write("TSIDが指定されました。tsid=" & tsid.ToString & " sid=" & sid.ToString & " chspace=" & ChSpace.ToString & "")
            End If
        End If
        ...
        Dim opt_TSID As String = ""
        If tsid > 0 Then
            opt_TSID = " /tsid " & tsid.ToString
        End If
        ...
        udpOpt = "/udp /port " & udpPortNumber & " /chspace " & ChSpace.ToString & " " & opt_serviceID & " " & opt_bondriver & opt_TSID
                    ...
                    ElseIf ISOPlayNEW = 0 Then
                        ...
                        If exepath_ISO_VLC.Length > 0 Then
                            ...
                            If resultInfo.ISO_RC = 0 Then
                                ...
                                ElseIf hlsOpt.ToLower.IndexOf("encc.exe") > 0 And hlsOpt.IndexOf("--vpp-sub") < 0 And subParam.Length > 0 Then
            ...
            If rUrl.IndexOf("/WatchTV") >= 0 Then
                ...
                If auth_ok > 0 Then
                    ...
                    If rUrl.IndexOf("?") > 0 And rUrl.IndexOf("=") > 0 Then
                        ...
                        Dim h_tsid As String = instr_pickup_para(rUrl, "TSID=", "&", 0)
                        ...
                        If h_bon_sid_ch_str.Length > 0 Then
                            ...
                            End If
                        End If
                        'chspaceにtsidが含まれいる場合
                        If Val(h_chspace) >= 100 Then
                            If h_tsid.Length = 0 Then h_tsid = Int(Val(h_chspace) / 100).ToString
                            h_chspace = Val(h_chspace) Mod 100
                        End If
                        ...
                        If ffmpeg_num > 0 And ((h_stream_mode = 2 And h_bondriver.Length > 0 And Val(h_sid) > 0) Or (h_stream_mode = 3 And h_videoname.Length > 0)) Then
                            ...
                            Dim chspace_add_tsid As Integer = Val(h_chspace)
                            If TSID_in_ChSpace = 1 Then
                                chspace_add_tsid = Val(h_tsid) * 100 + Val(h_chspace)
                            End If
                            Me.start_movie(ffmpeg_num, h_bondriver, Val(h_sid), chspace_add_tsid, Me._udpApp, Me._hlsApp, Me._hlsOpt1, Me._hlsOpt2, Me._wwwroot, Me._fileroot, Me._hlsroot, Me._ShowConsole, Me._udpOpt3, h_videoname, h_NHK_dual_mono_mode_select, h_stream_mode, h_resolution, h_VideoSeekSeconds, h_nohsub, h_baisoku, h_hlsOptAdd, h_margin1, "", "", httpApp, Nothing)
                        Else
                            'パラメーターが不正
                ...
                Try
                    ...
                    If auth_ok > 0 Then
                        ...
                        If pext.IndexOf(".htm") >= 0 Or (path.IndexOf("WI_") >= 0 And pext = ".json") Then
                            ...
                            'TSID
                            Dim tsid As String = System.Web.HttpUtility.ParseQueryString(req.Url.Query)("TSID") & ""
                            'chspace指定
                            ...
                            End If
                            'chspaceにtsidが含まれていた場合
                            If Val(chspace) >= 100 Then
                                If tsid.Length = 0 Then tsid = Int(Val(chspace) / 100).ToString
                                chspace = Val(chspace) Mod 100
                            End If
                            '解像度指定 "640x360"等
                            ...
                            ElseIf req_Url.ToLower = ("/StartTv.html").ToLower Then
                                ...
                                If num > 0 And bondriver.Length > 0 And Val(sid) > 0 And Val(chspace) >= 0 Then
                                    ...
                                    Dim chspace_add_tsid As Integer = Val(chspace)
                                    If TSID_in_ChSpace = 1 Then
                                        chspace_add_tsid = Val(tsid) * 100 + Val(chspace)
                                    End If
                                    Me.start_movie(num, bondriver, Val(sid), chspace_add_tsid, Me._udpApp, Me._hlsApp, Me._hlsOpt1, Me._hlsOpt2, Me._wwwroot, Me._fileroot, Me._hlsroot, Me._ShowConsole, Me._udpOpt3, videoname, NHK_dual_mono_mode_select, stream_mode, resolution, 0, 0, "1", hlsOptAdd, 0, hlsAppSelect, profileSelect, httpApp, Nothing)
                                    'すぐさま視聴ページへリダイレクトする
