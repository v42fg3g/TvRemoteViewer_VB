...
■5ch連続読み込み規制につきまして
	いつかこのような時が来るとは思っていましたがいざとなると怖いものです。手の震えが止まりません・・
	・iniの関連項目
		Ch5_read_dat			1にすると、スレURLをdatのURLに変換して取得を試みます
		Ch5_Proxy_Server		稼働中のdat取得可能なプロキシサーバーURL
		Ch5_UserAgent			User-Agent
	・特殊な5chスレdat取得可能プロキシが稼働している環境
		ini推奨設定例
			Ch5_read_dat = 1(datのみプロキシ経由) または 2(dat以外もプロキシを経由する)
			Ch5_Proxy_Server = http://127.0.0.1:8080
				（プロキシソフトによる。インターネットオプションでプロキシを指定もしくは
					pacを指定している場合は空白でよいと思われます）
		5chのソフトウェア板に行くと公式対応以外の専ブラを使用するための特殊なプロキシのスレがあります
		（プロクシやproxyでスレ検索するとヒットします）
		いくつか種類があるのですが、apiによるdat取得に対応しているものを選択してインストールしてください
		（htmlを取得してdatに変換する方式は使えないと思われます。両方可能な場合はapi取得に設定してください）
			※今一番簡単と紹介されていたものを試してみました（20191110版）
			  そのままでは取得できませんでしたが
			  動作設定タブの「dat取得時にgZip圧縮で返答」チェックを外すと取得できました
  			  なおローカルPC専用です（LAN上からの使用は想定されていないようです）
...
■履歴
	...
	2.93	キャッシュ有効時の先読み機能を実装（ページ表示の高速化。待たされることがほぼなくなります）
			...
			5ch実況機能が使用できなかった不具合に対処。5ch実況用dat取得プロキシに対応（2.93t）
			...
			番組表キャッシュのバグを修正（長期間稼働で例外エラーが発生）（2.93x）
